...
def _download(path: str, url: str):
    ...
    if (not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0) and not os.path.exists(path):
        print(f"Iteration {os.environ.get('NEMO_ITERATION')}: downloading on LOCAL_RANK {os.environ.get('LOCAL_RANK')} from {url}")
        ...
        print(f"Iteration {os.environ.get('NEMO_ITERATION')}: NOT downloading on LOCAL_RANK {os.environ.get('LOCAL_RANK')} from {url}, file exists")
